...
def delete_file():
    file_path='data/newsChinaDaily.csv'
...
def saveData(urls,texts):
    print(urls)
    print("save...")
    with open('data/newsChinaDaily.csv', mode='w', newline='', encoding='utf-8') as file:
...
def getData(baseurl):
    ...
    for i in range(1,1+totalpage):
        ...
        #逐一解析数据
        soap=BeautifulSoup(html,'html.parser')
        for item in soap.find_all('div',attrs={'class':'busBox3'}):
            item=str(item)
            pattern = re.compile(r'<h3><a href="(.*?)".*?>(.*?)</a></h3>', re.S)
